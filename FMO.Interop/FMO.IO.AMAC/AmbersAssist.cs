...
public static class AmbersAssist
{
    ...
    public static async Task<bool> UploadQuartlyInvestor(params FundQuarterlyUpdate[] reports)
    {
        ...
        try
        {
            ...
            foreach (var item in query)
            {
                ...
                var sheet = item.File.File.GetFullPath();


                await UploadInvestorSheetAndZip(page, sheet, []);
    ...
    /// 打包风险揭示书
    /// </summary>
    private static void PackDiscloseSheets(List<Investor> data, int FundId, string FundName, DateOnly PeriodEnd)
    {
        using var db = DbHelper.Base();
